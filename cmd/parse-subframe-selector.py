#!/usr/bin/env python3
"""
This script parses a CSV file with subframes generated by PixInsight.
This was written in 2020 and PixInsight has changed a lot since then.

To generate CSV file, do the following:
- In PixInsight, open menu Process->ImageInspection->SubframeSelector
- Add files you want to inspect
- Click apply global (F6) - little round icon at the bottom of the window
- A new window SubframeSelector | Measurements will open
- Click "export table as CSV" button
"""

from hevelius import iteleskop
import sys
import argparse

# This is an ugly hack. It should be removed.
sys.path.append(".")

try:
    from hevelius import db
except ImportError:
    print("Make sure you have config.py filled in. Please copy config.py-example to config.py and fill it in.")
    sys.exit(-1)


def parse_task(line: str) -> dict:
    """
    Parses a string like this:
    Subframe,1,1,"E:/astro/nerpio/MTOA/2020Q2/SFDB_2020-05-12_00-39-28_J131878_MTOA_CV_1x1_0300s_NGC2403.fit",8.744279e-01,2.505002e+00,7.740531e-01, \
    8.744279e-01,2.760000e+02,1.622958e+01,1.735582e+01,400,3.436380e-02,9.298809e-01,2.496715e+00,9.148272e-02,2.159095e-02,"2020-05-11 22:39:28"
    """

    f = line.split(",")
    t = {}
    fname = f[3]

    t["filename"] = fname
    t["weight"] = float(f[4])
    t["fwhm"] = float(f[5])
    t["eccentricity"] = float(f[6])
    t["snrweight"] = float(f[7])
    t["median"] = float(f[8])
    t["meandeviation"] = float(f[9])
    t["stars"] = float(f[11])

    # Now try to get the job id from the filename. First, ignore the path...
    details = iteleskop.parse_iteleskop_filename(fname)
    t["id"] = details["task_id"]

    return t


def read_csv(fname: str) -> list:
    """
    Read specified files and returns a list of tasks
    """
    with open(fname, encoding="utf-8") as f:
        content = f.readlines()

    tasks = []
    found = False
    i = 0
    for line in content:
        i += 1
        line = line.strip()
        if not found and line.find("SubframeHeader") == -1:
            continue
        if not found:
            found = True
            print("Data header found in line %d" % i)
            continue

        tasks.append(parse_task(line))

    return tasks


def cmd_subframe_selector(args):
    """
    Parses CSV file with subframes.
    """

    print("Loading file %s" % args.subframe_selector)

    tasks = read_csv(args.subframe_selector)

    print("Found %d task(s)" % len(tasks))

    cnx = db.connect()

    cnt = 0
    for t in tasks:
        print("Task %d of %d: " % (cnt, len(tasks)), end="")
        cnt += 1
        if not args.dry_run:
            if t["id"] > 0:
                db.task_update(cnx, t["id"], t["fwhm"], t["eccentricity"])
            else:
                print("WARNING: skipping line %d, because task id is %d" %
                      (cnt, t["id"]))
        else:
            print("Pretending to update task %d with fwhm=%f, eccentricity=%f" % (
                t["id"], t["fwhm"], t["eccentricity"]))

    cnx.close()


if __name__ == '__main__':

    parser = argparse.ArgumentParser("Hevelius Processor 0.1.0")
    parser.add_argument("-s", "--subframe-selector",
                        help="SubframeSelector output CSV file.", type=str, required=True)
    parser.add_argument("-d", "--dry-run", help="Pretends to do updates.",
                        action='store_true', default=False, required=False)

    arguments = parser.parse_args()

    cmd_subframe_selector(arguments)
